...
- blocking 同步阻塞式I/O
	...
	1. 通过多线程/进程模式**解决了多连接阻塞问题**，连接用的accept函数在主进程中，数据交互函数（recv和send）在子线程/进程中，不会阻塞连接也不会阻塞其他子线程/进程收发数据
	2. 多线程/进程模式下，大量连接情况下会导致开销问题
	3. 线程/进程池模式，解决了开销问题，但同样存在“池”大小选择问题
	4. 上面两种模式，没有考虑非阻塞I/O，因为没有必要
